...
- **CI**: видалено мертві посилання на Linux-артефакт з upload-списку (Linux більше не збирається), спрощено публікацію Windows-артефакту лише до інсталятора; підпис (Authenticode) застосовується безпосередньо до інсталятора
- **Безпека (fix): виправлено junction/symlink-вектор довільного видалення файлів** — сканер більше не спускається в reparse points (junction/символічні посилання). Раніше junction у сканованому корені (напр. у `%TEMP%`) міг вказувати на довільну директорію (напр. користувацькі Documents), файли якої класифікувалися як «temp cache» і видалялися. Тепер будь-який reparse point при обході вважається жорстким prune; додано регресійний тест
- **Надійність (fix)**: `SignatureInspector` тепер споживає stderr PowerShell-процесу — запобігає блокуванню процесу через заповнення pipe-буфера при великій кількості Advanced-кандидатів
- **Валідація (fix)**: поріг «нещодавнього використання» при очищенні винесено в `CleanupOptions.RecencyDays` (за замовчуванням 3, як і раніше) замість хардкоду в `CleanupEngine` — узгоджується з фільтром сканера
- **Тести**: 165 pass (додано регресійний тест на не-спуск у reparse points)
